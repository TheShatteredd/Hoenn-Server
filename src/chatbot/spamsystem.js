...
var spamphase = require('./bot.js').bot().spamphase;
	if(spamphase === 0) {
		return true;
	}
	else if(spamphase === 1) {
    ...
    if (message.indexOf(filter[i]) > -1) {
        user.lock();
        room.add('|html|<font color="#FF00BF"><i><b>' + bot.modbot + '</b> locked ' + user.name + '(somthing bad :P).</i></font>');
        return false;
    }
}
    ...
    if (user.numMessages == 15) {
        ...
        room.add('|html|<font color="#FF00BF"><i><b>' + bot.modbot + '</b> has muted ' + user.name + ' for 7 minutes(flood).</i></font>');
        user.o3omessagetime = today.getMinutes();
        user.numMessages = 0;
        return false;
    }
  ...
  //caps
			...
			for (var i=0;i<alpha.length;i++) {
				if(message.indexOf(alpha[i]) >= 0) {
					if (message === message.toUpperCase() && message.toUpperCase >= 6) {
						...
						user.warnCounter += 1;
						room.add('|html|<font color="#FF00BF">'+user.name+' was warned by '+'<i><b>' + bot.modbot + '</b> '+'.' +  ' (caps)</i></font>');
    ...
    if (user.warnCounters > 4) {
        room.add('|html|<font color="#FF00BF">' + user.name + ' was muted by ' + '<i><b>' + bot.modbot + '</b>(more than 4 warnings)</i></font>');
 ...
 if(spamphase === 2) {
	...
	if (message.toLowerCase().indexOf("psim") > -1) {
        ...
        return false;
        user.warnCounters += 1;
        room.add('|html|<font color="#FF00BF">' + user.name + ' was warned by ' + '<i><b>' + bot.modbot + '</b>(advertising)</i></font>');
    ...
    var filter = ['meat spin', 'meatspin','pornhub','porn hub','4chan.org','xxx.com','420yolo','fag','fgt','feg','spamspam','cum','cumshot','nigger','snen snen','cock','c0ck','anal','fag','feg','penis'];
	for (var i=0;i<filter.length;i++) {
    if (message.indexOf(filter[i]) > -1) {
        user.lock();
        room.add('|html|<font color="#FF00BF"><i><b>' + bot.modbot + '</b> locked ' + user.name + '(somthing bad :P).</i></font>');
	...
	if (message.toLowerCase().indexOf("psim") > -1) {
        this.add(bot.modbot + 'has banned ' + user.name + 'for advertising.');
        user.ban();
    }
    if (message.toLowerCase().indexOf("ps im.") > -1) {
        this.add(bot.modbot + 'has banned ' + user.name + 'for advertising.');
    ...
    if (message.toLowerCase().indexOf(".prism.us") > -1) {
    	this.add(bot.modbot + 'has banned ' + user.name + 'for advertising.');
        user.ban();
    }	
    if (user.numMessages == 10) {
    ...
    if (bot.spammers.indexOf(user.userid) > -1) {
    	...
    	if (user.warnCounters > 1) {
        room.add('|html|<font color="#FF00BF">' + user.name + ' was muted by ' + '<i><b>' + bot.modbot + '</b>(more than 4 warnings)</i></font>');
        user.mute(room.id, 60 * 60 * 1000, true);
        return false;
    }
    var filter = ['meat spin', 'meatspin','pornhub','porn hub','porn','redtube','red tube','imgur','4chan.org','xxx.com','420yolo','fag','fgt','feg','spamspam','cum','cumshot','nigger','snen snen','cock','c0ck','anal','fag','feg','penis'];
	for (var i=0;i<filter.length;i++) {
    if (message.indexOf(filter[i]) > -1) {
        user.lock();
        room.add('|html|<font color="#FF00BF"><i><b>' + bot.modbot + '</b> locked ' + user.name + '(somthing bad :P).</i></font>');
