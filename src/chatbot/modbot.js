...
var joinAllRooms = require('./settings').settings.modbot.autoJoinAll;
var joinRooms = require('./settings').settings.modbot.rooms;
var path = require('path');
  
var worker = new (require(path.join(__dirname, './', 'fake-process.js')).FakeProcess)();
	Users.socketConnect(worker.server ,undefined, '1', '76.19.156.128');

	// Getting the fake user from the Users list
	for (var i in Users.users) { 
		if(Users.users[i].connections[0].ip === '76.19.156.128') {

			var Bot = Users.users[i];

			// Modifying user's properties
			Bot.name = bot.modbot.substr(1,bot.name.length);
			Bot.named = true;
			Bot.renamePending = Bot.name;
			Bot.authenticated = true;
			Bot.userid = toUserid(Bot.name);
			Bot.group = bot.modbot.substr(0,1);
